...
//	//GameWindow embedWindow(800, 600, false);
    gameWindow->use_context();
    Engine::set_game_window(gameWindow);
    LOG(INFO) << "Hellooooo\n";

    //Create the interpreter
    Interpreter interpreter(boost::filesystem::absolute("python_embed/wrapper_functions.so").normalize());
    //Create the input manager
    InputManager* input_manager = gameWindow->get_input_manager();

    //Create the GUI manager
    GUIManager gui_manager;

//    //Create the map viewer
    MapViewer map_viewer(gameWindow, &gui_manager);
...
//
    ...
    std::shared_ptr<Text> stoptext = std::make_shared<Text>(gameWindow, buttonfont, true);
    std::shared_ptr<Text> runtext = std::make_shared<Text>(gameWindow, buttonfont, true);
    ...
    auto original_window_size = gameWindow->get_size();
    ...
    Lifeline gui_resize_lifeline = gameWindow->register_resize_handler(gui_resize_func);


    //The callbacks
    // WARNING: Fragile reference capture
    Lifeline map_resize_lifeline = gameWindow->register_resize_handler([&] (GameWindow *)
    {
        map_viewer.resize();
    });
...
/*
    ...
    Text tile_identifier_text(gameWindow, Engine::get_game_font(), false);
    ...
    Lifeline text_lifeline_char = gameWindow->register_resize_handler(func_char);
