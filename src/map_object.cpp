...
/*MapObject::MapObject(std::string name): name(name) { TODO: Remove
...
void MapObject::set_id(int new_id) {
    id = new_id;
}

void MapObject::set_name(std::string new_name) {
    name = new_name;
}
...
void MapObject::set_object_status(std::string _object_status) {
    ...
    if (!status_icon) {
        LOG(ERROR) << "MapObject manager no longer has status_icon";
        return;
    }
...
void MapObject::set_focus(bool _is_focus) {
    ...
    if (_is_focus != is_focus) {
        ...
        if (!focus_icon) {
            LOG(ERROR) << "MapObject manager no longer has focus_icon";
            return;
        }
