...
static void draw_square () {
  ...
  printf ("Test: Texture load times, averaged over 64 loads.\n");

  for (res = 1; res < GL_MAX_TEXTURE_SIZE; res<<=1) { //res set to zero in block
    textureData = malloc (4*res*res); //RGBA * width * height
    clock_gettime (CLOCK_REALTIME, &timeBefore);
    for (i = 0; i < 64; i++) {
      glGenTextures(1, &texture);
      glBindTexture(GL_TEXTURE_2D, texture);
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, res, res, 0,
    ...
    printf ("Times: %i %i %i %i %f\n", timeAfter.tv_sec, timeAfter.tv_nsec, timeBefore.tv_sec, timeBefore.tv_nsec, (double)((long)timeAfter.tv_sec - (long)timeBefore.tv_sec));
    if (texture != 0) {
      printf ("Time to process %ix%i was %fs\n", res, res,
	      (
	       (double)(timeAfter.tv_sec - timeBefore.tv_sec)
	       + ((double)(timeAfter.tv_nsec - timeBefore.tv_nsec) / 1000000000.0f)
	       ) / 64
	      );
      fflush (stdout);
    }
