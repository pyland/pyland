...
def start(script_api, script_name, script_state_container, engine, parse_error = True, callback = None):
    ...
    def parse_output(error_message):
        ...
        if ("NameError" in error_message):
        ...
        elif ("SyntaxError" in error_message) and ("print" in error_message) and ("EOL" in error_message):
            ...
            parsed_error_message = ("PyRunner Error: There is no end quotation mark for the \n'print' command in line "+ line + "\n"+error_pointer)
        elif ("SyntaxError" in error_message) and ("print" in error_message) and ("EOF" in error_message):
            #Get the section of the error after the second line number has been given
            #(this section shows an arrow to the part of the line with the issue)
            error_section_string = parse_line_number_index(error_message, 2)[2]
            error_start_index = parse_line_number_index(error_message, 2)[1]
            ...
            parsed_error_message = ("PyRunner Error: Syntax error in line "+ line + "\n"+error_pointer)
